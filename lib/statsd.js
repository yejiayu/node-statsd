 ...
 * @constructor
 */
var Client = function (host, port, prefix, suffix, globalize, cacheDns, mock, global_tags, ttl) {
  var options = host || {},
         self = this;
  ...
  if(options.cacheDns === true){
    ...
    if (options.ttl && options.ttl % 1 === 0) {
      timingLookupDNS.call(this, this.host, options.ttl)
    }
  }
...
function timingLookupDNS(host, ms) {
  setTimeout(() => {
    dns.lookup(host, (err, address, family) => {
      if (err == null) {
        this.host = address
      }
      this.emit('changeDNS', address)
      timingLookupDNS(host, ms).bind(this)
    })
  }, ms);
}
